...
The script supports two directory layouts:
    ...
    Workspace layout (from codex-skill-creator iterations):
    <benchmark_dir>/
    └── eval-N/
        ├── with_skill/
        │   ├── run-1/grading.json
...
def load_run_results(benchmark_dir: Path) -> dict:
    ...
    for eval_idx, eval_dir in enumerate(sorted(search_dir.glob("eval-*"))):
        ...
        if metadata_path.exists():
            try:
                eval_id = read_json_utf8(metadata_path).get("eval_id", eval_idx)
        ...
        for config_dir in sorted(eval_dir.iterdir()):
            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                ...
                try:
                    grading = read_json_utf8(grading_file)
                ...
                if timing_file.exists() and (result["time_seconds"] == 0.0 or not result["tokens"]):
                    try:
                        timing_data = read_json_utf8(timing_file)
